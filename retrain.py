...
g_bottleneck_values = {}  # 全局bottlenect map
    ...
    for sub_dir in sub_dirs:
        ...
        for file_name in file_list:
            ...
            percentage_hash = ((int(hash_name_hashed, 16) %
                               ...
                               (100.0 / MAX_NUM_IMAGES_PER_CLASS))  # consistent hash 的想法，对文件名做 hash 然后计算
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, image_dir,
    ...
    if not os.path.exists(bottleneck_path):
        ...
        image_data = gfile.FastGFile(image_path, 'rb').read()  # 读取某张图片数据  这里不管分辨率的问题？是不是 CNN 内部已经搞定了.
        bottleneck_values = run_bottleneck_on_image(sess, image_data,
                                                    jpeg_data_tensor,
                                                    bottleneck_tensor)  # 计算他的 bottlenect_tensor
        bottleneck_string = ','.join(str(x) for x in bottleneck_values)
        with open(bottleneck_path, 'w') as bottleneck_file:
            bottleneck_file.write(bottleneck_string)  # 将 bottlenect 写入到cache 文件中

    with open(bottleneck_path, 'r') as bottleneck_file:
        bottleneck_string = bottleneck_file.read()  # 再读出来返回
...
def cache_bottlenecks(sess, image_lists, image_dir, bottleneck_dir,
    ...
    for label_name, label_lists in image_lists.items():
        for category in ['training', 'testing', 'validation']:
            category_list = label_lists[category]  # 遍历所有 label 的所有 sample集合
            for index, unused_base_name in enumerate(category_list):  # 每张图 都计算
...
def get_random_distorted_bottlenecks(
    ...
    for unused_i in range(how_many):
        label_index = random.randrange(class_count)
        label_name = list(image_lists.keys())[label_index]  # 随机取label
        image_index = random.randrange(MAX_NUM_IMAGES_PER_CLASS + 1)  # 随机取 image
